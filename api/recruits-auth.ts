...
export default async function handler(request: Request): Promise<Response> {
  ...
  // Handle logout — now at /recruits/{slug}/logout so the
  // cookie Path matches and the browser accepts the clearing directive
  const logoutMatch = pathname.match(/^\/recruits\/([a-z0-9-]+)\/logout$/);
  if (logoutMatch) {
    return handleLogout(request, url, logoutMatch[1]);
  }
...
async function handleLogout(request: Request, url: URL, slug: string): Promise<Response> {
  const headers = new Headers();

  // Clear the auth cookie with exact Path match (required by browsers
  // to accept the Set-Cookie clearing directive)
  headers.append(
    'set-cookie',
    [
      `${COOKIE_NAME}=`,
