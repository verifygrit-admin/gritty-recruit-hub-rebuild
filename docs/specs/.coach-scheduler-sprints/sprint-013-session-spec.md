...
| Prototype HTML | `docs/specs/.coach-scheduler-sprints/prototype/index.html` | Visual reference for player picker step (Step 3 in the modal preview row) |
...
### D4 — `schools.meeting_location` Field

Add a `meeting_location` text field to the `schools` table to populate the .ics LOCATION field. Populate for BC High (e.g., "BC High Football Office, [address]"). Field defaults to school address if no specific meeting location is set.

### D5 — Server-Side ICS Generation

On submit, the server (Supabase Edge Function or equivalent) generates a single `.ics` file with:
...
Integrate transactional email provider (Resend recommended). On submit:
